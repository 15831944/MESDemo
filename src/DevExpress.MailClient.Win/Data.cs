...
namespace DevExpress.MailClient.Win
{
    ...
    public class DataHelper
    {
        ...
        internal static DataTable UserTable
        {
            ...
            {
                string table = "Users";
                DataTable users = CreateDataTable(table);
                //if (users != null && users.Rows.Count > 0)
                //    LastMailDate = (DateTime)users.Rows[0]["Date"];
                return users;
            }
        }

        internal static DataTable PlanTable
        {
            get
            {
                string table = "Plans";
                DataTable plan = CreateDataTable(table);
                //if (users != null && users.Rows.Count > 0)
                //    LastMailDate = (DateTime)users.Rows[0]["Date"];
                return plan;
            }
        ...
        public static List<User> User
        {
            get
            {
                try
                {
                    ...
                    {
                        user = new List<User>();
                        DataTable tbl = UserTable;
                        if (tbl != null)
                        {
                            foreach (DataRow row in tbl.Rows)
                                user.Add(new User(row));
                        }
                        user.Add(CreateUserAbout());
                    }
                }
                catch (Exception e)
                {
    ...
    public class User
    {
        ...
        string username = string.Empty, sex = string.Empty, realname = string.Empty,
            email = string.Empty, deptname = string.Empty, phonenumber = string.Empty,
            state = string.Empty;
        int id = 0;
        ...
        public User(DataRow row)
        {
            ...
            this.username = string.Format("{0}", row["UserName"]);
            this.realname = string.Format("{0}", row["RealName"]);
            this.sex = string.Format("{0}", row["Sex"]);
            this.deptname = string.Format("{0}", row["DeptName"]);
            this.phonenumber = string.Format("{0}", row["phonenumber"]);
            this.state = string.Format("{0}", row["State"]);
            this.email = string.Format("{0}", row["Email"]);
            //this.from = string.Format("{0}", row["From"]);
            //this.subject = string.Format("{0}", row["Subject"]);
            //this.isReply = (bool)row["IsReply"];
            //this.hasAttachment = (bool)row["HasAttachment"];
